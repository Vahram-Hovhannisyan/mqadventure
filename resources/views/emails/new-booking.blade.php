...
<div class="wrap">
    <div class="header">
        <h1>Новая заявка #{{ $booking->id }}</h1>
        <p style="margin:4px 0 0; opacity:.8; font-size:14px;">Meghradzor Quad Adventure</p>
    </div>
    <div class="body">
        <div class="row"><span class="label">Имя</span><span class="value">{{ $booking->name }}</span></div>
        <div class="row"><span class="label">Телефон</span><span class="value">{{ $booking->phone }}</span></div>
        <div class="row"><span class="label">Тур</span><span class="value">{{ $tour }}</span></div>
        <div class="row"><span class="label">Дата</span><span class="value">{{ $booking->date?->format('d.m.Y') ?? 'Не указана' }}</span></div>
        <div class="row"><span class="label">Людей</span><span class="value">{{ $booking->people }}</span></div>
        @if($booking->comment)
            <div class="row"><span class="label">Комментарий</span><span class="value">{{ $booking->comment }}</span></div>
        @endif
        <div class="row"><span class="label">Получена</span><span class="value">{{ $booking->created_at->format('d.m.Y H:i') }}</span></div>
